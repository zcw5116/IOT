...
import utils.HbaseUtil.createIfNotExists
...
/*    val mdnsql= "select m.msisdn as mdn, case when m.pcause='0x0000' then 'success' else m.pcause end as pcause, count(*) as req_cnt, sum(case when pcause='0x0000' then 1 else 0 end ) as reqsucess_cnt " +
 ...
 /*   val tmpmdntable = "tmpmdntable"
    sqlContext.sql( s"drop table if exists ${tmpmdntable}")
    ...
    // 创建表, 如果表存在， 自动忽略
    createIfNotExists(htable,families)
...
/*
    val failedmmesql= "select m.msisdn as mdn, m.mmegid, m.mmecode, m.enbid " +
      " from iot_mme_mm_hw m  " +
    ...
    */
    ...
    // 创建表, 如果表存在， 自动忽略
    createIfNotExists(hfailedtable,failedfamilies)
